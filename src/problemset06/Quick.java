...
public class Quick {
	...
	private static void sort(Comparable[] a, int lo, int hi) {
		...
		if (size <= 3) // manual sort if small
			...
			double mid = median(a, lo, hi);
			if (hi <= lo)
				return;
			int j = partition(a, lo, hi, mid);
			sort(a, lo, j - 1);
			sort(a, j + 1, hi);
	...
	private static int partition(Comparable[] a, int lo, int hi, double mid) {
		int i = lo;
		int j = hi - 1;
		Comparable v = a[lo];
		while (true) {
	...
	public static int median(Comparable[] a, int left, int right) {
		...
		if (a[center].compareTo(a[center]) > 0) {
			exch(a, center, right);
		}
	...
	public static void manualSort(Comparable[] a, int left, int right) {
		int size = right - left + 1;
		if (size <= 1)
			return; // no sort necessary
		if (size == 2) { // 2-sort left and right
			if (a[left].compareTo(a[right - 1]) > 0)
				exch(a, left, right);
			return;
		} else // size is 3
		{ // 3-sort left, center, & right
			...
			if (a[right - 1].compareTo(a[right]) > 0) {
				exch(a, right - 1, right);
			}
